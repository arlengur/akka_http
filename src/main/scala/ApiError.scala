...
object ApiError {
  ...
  val emptyTitleField: ApiError = new ApiError(StatusCodes.BadRequest, "The title field does not be empty.")

  def todoNotFound(id: String): ApiError = new ApiError(StatusCodes.NotFound, s"The todo with id $id could not be found.")
}
